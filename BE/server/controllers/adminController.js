...
const reviewWorker = async (req, res) => {
  ...
  const updateServicePrice = async (req, res) => {
    try {
      ...
      if (service.code === 'cleaning'|| service.code === 'cleaning_ac') {
        ...
        return res.status(400).json({ success: false, message: 'Loại dịch vụ không hỗ trợ cập nhật giá này' });
      }
  ...
  const getServiceRevenue = async (req, res) => {
    try {
      ...
      res.status(500).json({ success: false, message: 'Lỗi khi tính tổng doanh thu theo dịch vụ', error });
    }
  };
  ...
  const getMostBookedService = async (req, res) => {
    try {
      ...
      res.status(500).json({ success: false, message: 'Lỗi khi tìm dịch vụ được đặt nhiều nhất', error });
    }
  };
